...
def read_excel_rows(excel_path, sheet_name=None, start_row=2):
    ...
    for r in range(start_row, ws.max_row + 1):
        ...
        check_cells = [
            ...
            # ✅ facilities ใหม่: AD,AE,AF,AG
            ws[f"AD{r}"].value, ws[f"AE{r}"].value, ws[f"AF{r}"].value, ws[f"AG{r}"].value,

            # ✅ disasters ใหม่: AH,AI,AJ,AK
            ws[f"AH{r}"].value, ws[f"AI{r}"].value, ws[f"AJ{r}"].value, ws[f"AK{r}"].value,
        ]
        ...
        row = {
            ...
            # ⚠️ เดิมคุณเคยใช้ AF เป็น responsibleAgency แต่ตอนนี้ AF = PET แล้ว (facility)
            # เลยตั้งเป็น "-" เพื่อไม่ชนกัน
            "responsibleAgency": None,
            ...
            # ✅ disasters flags: AH,AI,AJ,AK
            "dis_ah": is_has(ws[f"AH{r}"].value),  # FLOOD
            "dis_ai": is_has(ws[f"AI{r}"].value),  # STORM
            "dis_aj": is_has(ws[f"AJ{r}"].value),  # FIRE
            "dis_ak": is_has(ws[f"AK{r}"].value),  # EARTHQUAKE

            # ✅ facilities flags: AD,AE,AF,AG
            "fac_ad": is_has(ws[f"AD{r}"].value),  # ELECTRICITY
            "fac_ae": is_has(ws[f"AE{r}"].value),  # TENT
            "fac_af": is_has(ws[f"AF{r}"].value),  # PET
            "fac_ag": is_has(ws[f"AG{r}"].value),  # KITCHEN
        }
        rows.append(row)
...
def insert_shelter(cur, shelter_table, data, region_value: str | None):
    ...
    payload = {
        ...
        # ✅ ถ้าไม่ได้ map จาก excel ให้เป็น "-"
